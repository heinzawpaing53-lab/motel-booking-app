...
// If not logged in, store form data in session and redirect to login
if (!isLoggedIn()) {
    $_SESSION['reservation_data'] = $_POST ?: $_GET;
    $_SESSION['redirect_after_login'] = 'reservation.php';
    redirect('login.php');
}

$roomId = isset($_GET['room_id']) ? (int)$_GET['room_id'] : (isset($_POST['room_id']) ? (int)$_POST['room_id'] : 0);
$room = null;
if ($roomId) {
...
<section class="py-16 bg-gray-50">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <?php if (isset($_SESSION['reservation_data']) && !$_POST): ?>
        <div class="bg-yellow-100 border-l-4 border-yellow-500 text-yellow-700 p-4 mb-6 rounded-lg">Please fill in the details below to book your room.</div>
        <?php unset($_SESSION['reservation_data']); ?>
        <?php endif; ?>
        ...
        <form method="POST" class="bg-white rounded-xl shadow-sm p-8">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div>
                    ...
                    <input type="date" name="check_in" id="check_in" value="<?php echo $_POST['check_in'] ?? date('Y-m-d', strtotime('+1 day')); ?>" min="<?php echo date('Y-m-d'); ?>" class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required>
                </div>
                <div>
                    <label class="block text-sm font-semibold text-gray-600 mb-1">Check-out Date</label>
                    <input type="date" name="check_out" id="check_out" value="<?php echo $_POST['check_out'] ?? date('Y-m-d', strtotime('+2 days')); ?>" min="<?php echo date('Y-m-d', strtotime('+1 day')); ?>" class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" required>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div>
                    ...
                    <select name="adults" class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none">
                        <?php for($i=1;$i<=5;$i++): ?>
                        <option value="<?php echo $i; ?>" <?php echo ($_POST['adults']??1)==$i?'selected':''; ?>><?php echo $i; ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                <div>
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3 mb-6">
                ...
                <label class="flex items-center space-x-2 p-3 border border-gray-200 rounded-lg hover:bg-blue-50 cursor-pointer">
                    <input type="checkbox" name="requests[]" value="<?php echo $req['request_id']; ?>" class="rounded text-blue-600">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div class="p-4 border border-gray-200 rounded-lg">
                    ...
                    <select name="early_check_in_time" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                        ...
                        <?php for($h=6;$h<=13;$h++): ?>
                        <option value="<?php echo sprintf('%02d:00', $h); ?>" <?php echo ($_POST['early_check_in_time']??'')==sprintf('%02d:00', $h)?'selected':''; ?>><?php echo sprintf('%02d:00', $h); ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
                <div class="p-4 border border-gray-200 rounded-lg">
                    ...
                    <select name="late_check_out_time" class="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm">
                        ...
                        <?php for($h=12;$h<=18;$h++): ?>
                        <option value="<?php echo sprintf('%02d:00', $h); ?>" <?php echo ($_POST['late_check_out_time']??'')==sprintf('%02d:00', $h)?'selected':''; ?>><?php echo sprintf('%02d:00', $h); ?></option>
                        <?php endfor; ?>
                    </select>
                </div>
            </div>
            ...
            <div class="mb-8">
                ...
                <textarea name="special_notes" rows="3" class="w-full px-4 py-3 border border-gray-200 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none" placeholder="Any additional requests or notes..."><?php echo $_POST['special_notes'] ?? ''; ?></textarea>
            </div>
            ...
            <button type="submit" class="btn-primary w-full text-lg py-4"><i class="fas fa-check-circle mr-2"></i>Confirm Reservation</button>
        </form>
    </div>
</section>
